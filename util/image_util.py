...
def un_normalize(tensor, img_format, pixel_dict):
    ...
    """
    #raise ValueError("pixel_dict的形状是{}".format(pixel_dict))
    tensor = tensor.repeat(3, 1, 1, 1)   #改：转成RGB3通道
...
def add_heat_map(pixels_np, intensities_np, alpha_img=0.33, color_map='magma', normalize=True):
    ...
    #raise ValueError("pixels_np的形状是{},heat_map的形状是{}".format(pixels_np.shape,heat_map.shape))
