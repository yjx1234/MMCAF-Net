...
class PENet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Encoders
        x_skips = []
        latent_fea=[]  #[4,256,6,48,48],[4,512,3,24,24],[4,1024,2,12,12]
        #raise ValueError("len是{}".format(len(self.encoders)))
        for i, encoder in enumerate(self.encoders):
            ...
            x = encoder(x)
            latent_fea.append(x) 
        #raise ValueError("fea1是{},fea2是{},fea3是{}".format(latent_fea[0].shape,latent_fea[1].shape,latent_fea[2].shape))
        # ASPP layer
        x = self.asp_pool(x)
